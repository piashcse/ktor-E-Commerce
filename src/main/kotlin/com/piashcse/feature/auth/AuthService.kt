...
class AuthService(
    ...
    override suspend fun login(loginRequest: LoginRequest): LoginResponse {
        ...
        val user =
            findUserByEmailAndType(loginRequest.email, userTypeEnum)
                ?: run {
    ...
    private suspend fun checkAccountLockout(
        email: String,
    ...
    private suspend fun findUserByEmailAndType(
        email: String,
        userTypeEnum: UserType,
    ): UserDAO? =
        query {
    ...
    override suspend fun resetPassword(resetPasswordRequest: ResetRequest): Int =
        query {
            ...
            // Find the specific user with the given email and userType
            val userEntity =
                userEntities.find { it.userType == userTypeEnum }
                    ?: throw NotFoundException(Message.Auth.userNotFoundForRole(resetPasswordRequest.email, resetPasswordRequest.userType))
     ...
     */
    private fun handleSellerTransition(
        user: UserDAO,
        newUserType: UserType,
    ) {
