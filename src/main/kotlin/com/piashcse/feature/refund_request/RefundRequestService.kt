...
interface RefundRequestRepository {
    ...
    suspend fun getRefundsByOrderId(orderId: String, userId: String, userType: UserType): List<RefundRequestResponse>
...
class RefundRequestService : RefundRequestRepository {
    ...
    override suspend fun getRefundsByOrderId(
        ...
        userId: String,
        userType: UserType
    ): List<RefundRequestResponse> = query {
        val order = OrderDAO.findById(orderId)
            ?: throw ValidationException(Message.Orders.NOT_FOUND)
        ...
        val refunds = RefundRequestDAO.find {
            RefundRequestTable.orderId eq EntityID(orderId, OrderTable)
        }.toList()

        refunds.map { it.response() }
    }
